...
func process(wind []byte) int {
	...
	for i := 0; i < count; i++ {
		...
		// the same pattern repeats every wind cycle
		if windIndex == 0 && i > 0 {
			if firstCycle == nil {
				firstCycle = &cycle{rocks: i, highestRock: ch.highestRock}
			} else {
				rocksInCycle := i - firstCycle.rocks
				highPerCycle := ch.highestRock - firstCycle.highestRock
				rocksLeft := count - i - 1
				fullCycles := rocksLeft / rocksInCycle
				skipped = fullCycles * highPerCycle
				i += fullCycles * rocksInCycle
			}
		}
	}

	return skipped + ch.highestRock
}
